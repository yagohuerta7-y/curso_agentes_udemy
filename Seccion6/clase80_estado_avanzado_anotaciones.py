# Hay casos en los que el modelado es mas complejo, y vas a querer que mas de un nodo actualice una misma clave del estado.
# Por defecto, se va a conservar la mas reciente (la actualizacion mas reciente sobreescribe la modificacion pasada del estado).
# Para poder combinar/acumular estos cambios, usaremos las funciones 'Reducer'. Esto es una funcion que toma un valor existente de una clave del estado,
# toma tambien el nuevo valor que toma un nodo, y produce un nuevo valor combinado.


# === Supongamos que, en la solucion del instructor, queremos agregar logs ===
...
class State(TypedDict):
    ...
    minutes: str
    summary: str
    logs: Annotated[list[str], add] # El tipo de dato que va a ser es una lista de cadenas de texto, y la vamos a combinar concatenandolo
